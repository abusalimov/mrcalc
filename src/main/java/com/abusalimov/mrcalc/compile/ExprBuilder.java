...
public class ExprBuilder<E extends Expr> implements NodeArgVisitor<E, ExprTypeInfo> {
    private final ExprFactory<E> exprFactory;

    public ExprBuilder(ExprFactory<E> exprFactory) {
    ...
    public E buildChild(ExprTypeInfo parent, ExprHolderNode node) {
        ExprTypeInfo eti = parent.getChild(node);
        return build(eti);
    }

    public E build(ExprTypeInfo eti) {
        return visit(eti.getExprNode(), eti);
    }

    public E visitCast(ExprNode resultNode, ExprNode nodeToVisit, ExprTypeInfo eti) {
    ...
    public E doVisit(VarRefNode node, ExprTypeInfo eti) {
        ...
        return getObjectMath(node, eti).load(name, eti.getReferencedVariableIndex(name));
    }
    ...
    private <T> ObjectMath<T, E, E> getObjectMath(Type exprType) {
        return exprFactory.getObjectMath((Class<T>) exprType.getTypeClass());
    }
    ...
    private <T extends Number> NumberMath<T, E, E> getNumberMath(Type exprType) {
        return exprFactory.getNumberMath((Class<T>) exprType.getTypeClass());
    }
    ...
    private NumberCast<E, E> getNumberCast(Type toType, Type fromType) {
        ...
        return exprFactory.getNumberCast(toPrimitive.getTypeClass(), fromPrimitive.getTypeClass());
    }
